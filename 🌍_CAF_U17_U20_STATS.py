...
# Layout setup
st.set_page_config(page_title='CAF Youth', layout='centered', page_icon='🌍')

# Tabs
tab3, tab4 = st.tabs(["Player Total Stats", "Player Maps"])
...
with tab3:
    ...
    def load_data(league_name):
        ...
        df = pd.read_excel(file_map[league_name])
        return df

    if st.button("Apply Filter", key="apply_stats"):
        st.session_state["df_stats"] = load_data(st.session_state["league3"])
        st.session_state["filter_applied3"] = True
...
with tab4:
    ...
    # Maintain selections across reruns using session state
    if "league4" not in st.session_state:
        st.session_state["league4"] = "CAF U17"
    if "filter_applied4" not in st.session_state:
        st.session_state["filter_applied4"] = False

    # League selector
    league = st.selectbox("Select League", ["CAF U17", "CAF U20"], key="league4")
    
    @st.cache_data
    def load_data_maps(league_name):
        file_map = {
            "CAF U17": "CAF U17 24-25.xlsx",
            "CAF U20": "CAF U20 24-25.xlsx"
        }
    ...
    def create_key_passes_map(df, player_name, team_name):
        ...
        ax.text(pitch_length*0.22, pitch_width + 12, f"{len(progressive_passes)} Progressive Passes",
    ...
    def create_shots_map(df, player_name, team_name):
        ...
        # --- Add logo ---
        logo = Image.open("TAP Logo.png").resize((150, 150))
    ...
    # Apply filter button
    if st.button("Apply Filter", key="apply_map_filter"):
        with st.spinner("🔄 Loading data, please wait ..."):
            st.session_state["df_maps"] = load_data_maps(st.session_state["league4"])
            st.session_state["filter_applied4"] = True

    # Show filters only after Apply Filter
    if st.session_state.get("filter_applied4", False):
        df = st.session_state["df_maps"]
        ...
        map_type = st.selectbox(
            "Select Map Type",
            ["Passes Received", "Dribbles Map","Key & Progressive Passes Map","Shots Map","Defensive Actions Map","Tackles Map","Recoveries & Interceptions Map"],
            key="map_type4"
        )
